...
class MemoryOptimizedAdaptiveHinaAdamW(AdamW8bit):
    """
    記憶體優化版本的自適應 HinaAdamW 優化器
    ...
    def __init__(
        ...
        **kwargs
        ...
        logger.info(f"MemoryOptimizedAdaptiveHinaAdamW 初始化完成，記憶體預算：{vram_budget_gb}GB")
    ...
    def step(self, closure=None):
        ...
        for group_idx, group in enumerate(self.param_groups):
            ...
            for param in group['params']:
                ...
                if grad.is_sparse:
                    raise RuntimeError('MemoryOptimizedAdaptiveHinaAdamW 不支援稀疏梯度')
                ...
                # AGR 正則化
                if self.use_agr:
                    grad = MemoryOptimizedAdaptiveHinaAdamW._apply_agr_regularization_optimized(grad)
                ...
                if self.use_cautious:
                    update = MemoryOptimizedAdaptiveHinaAdamW._apply_cautious_update_optimized(update, grad)
    ...
    def get_optimization_info(self) -> Dict[str, Any]:
        """獲取優化器的詳細信息"""
        info = {
            'optimizer_type': 'MemoryOptimizedAdaptiveHinaAdamW',
