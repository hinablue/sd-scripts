...
def fourier_latent_loss_multiscale(model_pred: torch.Tensor, target: torch.Tensor,
                                  scales: list = [1, 2], scale_weights: list = None,
...
def conditional_loss_with_fourier(
    ...
    fourier_scale_weights: list = None,
...
# 便利函數：預設配置
def get_fourier_loss_config(mode: str = "balanced") -> dict:
    """
    獲取預設的傅立葉損失配置

    Args:
        mode: 配置模式
            - "conservative": 保守配置，較小的傅立葉權重
            - "balanced": 平衡配置，中等的傅立葉權重
            - "aggressive": 激進配置，較大的傅立葉權重
